...
public:
	...
	BuildWorld& getBuild() { return *m_build; }
	SlvCube *getCurrentSolve() 
	{ 
		if (m_slvs == NULL) return NULL;
		return ((m_slvs->size() == 0)?(NULL):(m_slvs->at(m_nCurSlv))); 
	}
	int getCurrentSolveIndex() { return m_nCurSlv; }
	...
	{ 
		if (m_slvs == NULL) return 0;
		return m_slvs->size(); 
	}
	int getUpToStep() const { return m_nUpToStep; }

	bool isSlvEngineRunning() { return (m_sthread != NULL) && (m_sthread->fRunning); }
	const RunStats* getRunningStats();

	const Shape* getCurrentShape() { return m_shp; }
